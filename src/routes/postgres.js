...
function runMigrations() {
    return migrate(require("./migrations"));
}
...
function wrapSql(callable) {
    return (q, ...values) => {
        ...
        return callable.query(query.join(""), values)
...
const sql = wrapSql(pgpool);

async function sqlTransaction(func) {
    while (true) {
        ...
        try {
            ...
            await func((q, ...values) => {
                ...
                return client.query(query.join(""), values)
                    .then((result) => result.rows)
                    .catch(e => {
...
async function internalMigrate(migrations) {
    const client = await pgpool.connect();
    ...
    try {
        ...
        console.log("Obtaining migration lock");
        await sql`SELECT pg_advisory_lock(1)`;
        console.log("Got migration lock");
        ...
        for (let i = 0; i < insertedMigrations.length; ++i) {
            ...
            if (insertedMigration.id !== i + 1) {
                console.log(insertedMigrations);
                console.log(`Migration order -> ${insertedMigrations.map(u => u.id).join(",")}`);
        ...
        let lastExecutedMigrationId = insertedMigrations.length === 0 ? -1 : insertedMigrations[insertedMigrations.length - 1].id;
        ...
        for (const migrationId of migrationIds) {
            ...
            if (typeof migrationQuery === "string" && migrationQuery.includes("-- no transaction")) {
                try {
                    console.log(`Running migration ${migrationId} WITHOUT TRANSACTION...`);
                    ...
                    console.log(migrationQuery.toString());
                    throw e;
                }
            } else {
